...
_DEFAULT_SERVERS = [
    ("安信",        "59.36.5.11", 7709),
    ("国泰君安14",  "117.34.114.14", 7709),
    ("国泰君安15",  "117.34.114.15", 7709),
    ("国泰君安16",  "117.34.114.16", 7709),
    ("国泰君安17",  "117.34.114.17", 7709),
    ("国泰君安18",  "117.34.114.18", 7709),
    ("国泰君安20",  "117.34.114.20", 7709),
    ("国泰君安27",  "117.34.114.27", 7709),
                ...
                for i, (mkt, code) in enumerate(stock_list):
                    try:
                        ...
                        if n > 0:
                            ...
                            # 0 条不再计入 done —— 空结果通常是服务器
                            # 返回空（站点数据接口失效）或日期范围内无数据
                            self._stats["skipped"] += 1
                            print(f"  [{i+1}/{total}] {code}: 0 条"
                                  f"（服务器返回空或日期范围无数据），未写文件")
