//! 智能连接客户端 — 分层健康检查 + 本地缓存
//!
//! API 为 `TdxHqClient` 的**子集**：仅 `get_security_bars` / `get_security_quotes`
//! 两个数据接口（带自动重试与故障切换），外加连接与缓存管理
//! （`connect_to_any` / `probe_and_cache` / `cache_stats` / `clear_cache`）。
//! 分时、分笔、xdxr、全历史等其余接口不在其中，需要时请直接使用 `TdxHqClient`。
//! 连接策略与 `TdxHqClient` 不同:
//! - **快速初始连接**: 仅验证 TCP + 握手，不做 K 线健康检查
//! - **惰性健康检查**: 首次 K 线请求返回空时触发，自动切换服务器
//! - **本地缓存**: 记录成功/失败服务器，下次连接优先使用缓存
//! - **黑名单机制**: 连败≥3 或协议异常（K 线空/解析错）自动拉黑（24h 过期；
//!   连败条目成功即解除，协议条目只能等过期；候选全员拉黑时降级照常尝试）
//!
//! ## 使用场景
//!
...
//! |------|-------------|----------------|
//! | 初始连接 | 无健康检查 | 无健康检查 |
//! | K 线请求 | 空响应自动重试（日K及以上） | 空响应自动重试 + 惰性健康检查切服 |
//! | 服务器缓存 | 无 | 本地 JSON 缓存 |
//! | 黑名单 | 手动 `block_server` | 自动（连败≥3/协议异常）+ 全员拉黑降级 |
//! | 适用场景 | 网络稳定 | 网络不稳定 |
//!
//! ## 注意事项
//!
